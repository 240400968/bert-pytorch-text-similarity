...
class HUABEIDataset(Dataset):
    """
    Dataset class for huabei datasets.
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            max_hypothesis_length: An integer indicating the maximum length
                ...
                Defaults to None.
        """
        self.sent1_lengths = [len(seq) for seq in data[:,2]]
        self.max_sent1_length = max_sent1_length
        if self.max_sent1_length is None:
            self.max_sent1_length = max(self.sent1_lengths)

        self.sent2_lengths = [len(seq) for seq in data[:,3]]
        self.max_sent2_length = max_sent2_length
        if self.max_sent2_length is None:
            self.max_sent2_length = max(self.sent2_lengths)

        self.num_sequences = len(data[:,2])

        self.data = {"ids": [],
                    "sent1": torch.ones((self.num_sequences,
                                             self.max_sent1_length),
                                            dtype=torch.long) * padding_idx,
                     "sent2": torch.ones((self.num_sequences,
                                               self.max_sent2_length),
                                              dtype=torch.long) * padding_idx,
                     "labels": torch.tensor(list(data[:, 4]), dtype=torch.long)}

        for i, sent1 in enumerate(data[:,2]):
            self.data["ids"].append(i)
            end = min(len(sent1), self.max_sent1_length)
            self.data["sent1"][i][:end] = torch.tensor(sent1[:end])

            sent2 = data[:, 3][i]
            end = min(len(sent2), self.max_sent2_length)
            self.data["sent2"][i][:end] = torch.tensor(sent2[:end])
    ...
    def __getitem__(self, index):
        return {"id": self.data["ids"][index],
                ...
                "sent1_length": min(self.sent1_lengths[index],
                                      self.max_sent1_length),
                "sent2": self.data["sent2"][index],
                "sent2_length": min(self.sent2_lengths[index],
                                         self.max_sent2_length),
                "label": self.data["labels"][index]}
...
class TEXTSIMILARITYDataset(Dataset):
    """
    Dataset class for text_similarity datasets.
    """

    def __init__(self,
                 data, padding_size=128, padding_idx=0):
        """
        Args:
            data: A dictionary containing the preprocessed premises,
                hypotheses and labels of some dataset.
            ...
            max_hypothesis_length: An integer indicating the maximum length
                ...
                Defaults to None.
        """
        #  [ input_ids, token_type_ids, attention_mask, label, query_id, doc_id, sentence1, sentence2]
        self.num_sequences = len(data[:,0])
        self.data = {"ids": [],
                    "input_ids": torch.tensor(list(data[:, 0]), dtype=torch.long),
                    "token_type_ids": torch.tensor(list(data[:, 1]), dtype=torch.long),
                    "attention_mask": torch.tensor(list(data[:, 2]), dtype=torch.long),
                     "labels": torch.tensor(list(data[:, 3]), dtype=torch.long)}

        for i, sent1 in enumerate(data[:,0]):
            self.data["ids"].append(i)
